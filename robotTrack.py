	...
	def Robot_position(self):
		...
		for frame in self.camera.capture_continuous(self.rawCapture, format="bgr", use_video_port = True):
			...
			if len(cntsg) > 0 and len(cntsb) > 0 and len(cntsr) > 0:
				...
				print("Using last known positions g,b,r")
				centerg = self.ptsg[0]
				centerb = self.ptsb[0]
				centerr = self.ptsr[0]
			...
			#Optional for testing - comment out when not needed
			#print(centerg)
			#print(centerb)
			#print(centerr)
			
			#Measure distance between red & blue referenced to green & robot angle
			angle = np.arctan2(yr - yb, xr - xb) * 180 / np.pi #Robot Angle
			Cp = [0, 0] #robot centre array initialise
